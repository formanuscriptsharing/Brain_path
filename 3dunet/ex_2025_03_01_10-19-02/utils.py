...
class ADNIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 应用 transforms，先转换为 NumPy 数组（假设原始数据是 NumPy 数组）
        X1 = self.tensor_transforms(X1)/256
        X2 = self.tensor_transforms(X2)/256
        # 将单通道扩展为 3 通道
        # X1 = X1.repeat(3, 1, 1)  # 复制到 3 个通道
        # X2 = X2.repeat(3, 1, 1)  # 复制到 3 个通道
...
class MRIDataset(Dataset):
    ...
    def update_label_first(self, rid, j, age,age_hat1_, feature):
        """
        传入的rid，j，age是长度为n的向量，feature是一个n*d的矩阵。
        按照age的大小对feature，j，rid进行排序。
        之后对feature进行变点检测，用l2norm和pelt。
        然后得到变点个数K以及位置。
        """
        # Step 1: 按照age对rid, j, age, feature进行排序
        ...
        # Step 2: 对排序后的feature进行变点检测
        model = rpt.Pelt(model="l2", min_size=3).fit(feature)

        # 使用predict方法进行变点检测，返回变点列表

        # 初始化pen，逐步调整pen以找到接近40个区间的结果
        pen = 1
        detected_breakpoints = []
        while len(detected_breakpoints) < 30- 1: # 因为区间数 = 断点数 + 1
            detected_breakpoints = model.predict(pen=pen)
            pen /= 1.1  # 如果断点数少了，减少pen
        while len(detected_breakpoints) > 40: # 因为区间数 = 断点数 + 1
            detected_breakpoints = model.predict(pen=pen)
            pen *= 1.1  # 如果断点数少了，减少pen
