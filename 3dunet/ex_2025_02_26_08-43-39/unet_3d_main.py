...
#第一轮不变，学完第一轮之后，预测得到所有的rid以及j的特征
import matplotlib.pyplot as plt
import matplotlib.cm as cm
from matplotlib.colors import ListedColormap
def visualize_tsne(feature, clinical_age1_, epoch, save_path='./',str=False):
    """
    用 t-SNE 可视化特征，并根据临床年龄对颜色进行编码。

    参数:
    - feature: n*256 的 numpy 数组, 表示特征数据
    - clinical_age1_: 长度为 n 的 numpy 数组, 表示对应的临床年龄
    - epoch: 整数, 当前训练的 epoch 号，用于文件命名
    - save_path: 字符串, 保存图片的路径, 默认为当前目录
...
def Pretext(model_encode,model_decode, optimizer, Epochs, train_dataset, val_loader, test_loader, structure_loss_mult, age_loss_mult,
    ...
    for epoch in range(Epochs):
        if epoch<stage0_epoch:
            ...
            model_decode.train()
            train_loader = torch.utils.data.DataLoader(train_dataset,
                                                       batch_size=int(args.batch_size/2), shuffle=True, num_workers=32, pin_memory=True)
...
if __name__ == "__main__":
    ...
    wandb.init(
        ...
        config={
            ...
            "architecture": "(1, 2, 4, 8)",
            "resume":None#"/pub/liyifan/mri/3dunet/ex_2025_02_23_22-01-41/check_point_model_100_1_1.pth"

        }
    )
    # path_model = "/pub/liyifan/mri/3dunet/ex_2025_02_23_22-01-41/check_point_model_100_1_1.pth"
    ...
    # dataset
    train_dir = a + '/FS_2/processed_health_all/train/'
    ...
    test_loader = torch.utils.data.DataLoader(MRIDataset( test_dir, resize_dim,train=False,mci_ad=False),
                                              batch_size=args.batch_size, shuffle=False, num_workers=0)
    ...
    if test_result:
        ...
        path_model ="/pub/liyifan/mri/3dunet/ex_2025_02_25_12-04-18/check_point_model_90_1_1.pth"
        state_dict = torch.load(path_model)
        new_state_dict = {k.replace('.module.', '.'): v for k, v in state_dict.items()}
